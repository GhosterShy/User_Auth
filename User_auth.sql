...
GRANT ALL PRIVILEGES TO full_admin;
GRANT admin_role TO full_admin;
...
EXPLAIN PLAN FOR
SELECT * FROM sessions WHERE location = 'Canada';
...
UPDATE SYSTEM.users set username = 'User1' WHERE id = 1;
...
SELECT id, username, email, phone, last_login
FROM users
WHERE is_active = 1;
...
--Sequences
CREATE SEQUENCE users_seq2
START WITH 15
INCREMENT BY 1
NOCACHE
NOCYCLE;

INSERT INTO users (id, username, email, password_hash, phone, is_active, is_verified, created_at, updated_at)
VALUES (users_seq2.NEXTVAL, 'test_user', 'test@example.com', 'hash123', '1234567890', 1, 0, SYSDATE, SYSDATE);
